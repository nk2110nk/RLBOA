...
class TimeBasedNegotiator(AspirationNegotiator):
    ...
    def propose(self, state: MechanismState) -> Optional["Outcome"]:
        ...
        if self.reserved_value is not None and asp < self.reserved_value:
            return None
        return self.ordered_outcomes[max(0, self.n_outcomes - np.searchsorted(self.ordered_utils, asp) - 1)][1]
...
class AgentK(SAONegotiator):
    ...
    def accept_probability(self, state: MechanismState, offer: "Outcome") -> float:
        ...
        if 1 - pre_target2 < 1e-12:
            pre_target2 = 1 - 1e-12
        ratio = (deviation + 0.1) / (1. - pre_target)
        if math.isnan(ratio) or ratio > 2.0:
            ratio = 2.0
        ratio2 = (deviation + 0.1) / (1. - pre_target2)
    ...
    def propose(self, state: MechanismState) -> Optional[Outcome]:
        ...
        if size > 0:
            ...
            while search_util < self.bid_target + noise:
                ...
                search_util = self.utility_function(next_bid)
                loop += 1
        return next_bid
...
class HardHeaded(SAONegotiator):
    ...
    def respond(self, state: MechanismState, offer: "Outcome") -> "ResponseType":
        ...
        elif self._utility_function(offer) > self._utility_function(self.opponent_best_bid):
            self.opponent_best_bid = offer
        ...
        if self.first_round:
            ...
            if self._utility_function(self.offer_queue[0]) < self._utility_function(self.opponent_best_bid):
        ...
        if not self.offer_queue:
            ...
            if self.opponent_last_bid is not None and util <= self._utility_function(self.opponent_last_bid):
        ...
        if self.opponent_last_bid is not None \
                and (self._utility_function(self.opponent_last_bid) > self.lowest_yet_utility
                     or self._utility_function(self.offer_queue[0]) <= self._utility_function(self.opponent_last_bid)):
            new_action = ResponseType.ACCEPT_OFFER
        else:
            new_action = self.offer_queue.pop(0)
            self.bid_history.append(new_action)
            if self._utility_function(new_action) < self.lowest_yet_utility:
                self.lowest_yet_utility = self._utility_function(new_action)
            # # ノイズsigma=0.005
            # if self.add_noise:
...
class CUHKAgent(SAONegotiator):
    ...
    def calculate_bids_between_utility(self):
        ...
        if self.maximumOfBid < 20000:
            ...
            while limits <= 20000:
                ...
                for i in range(maximum_rounds):
                    if i * 0.01 + min_utility <= self.utility_function(b) <= (i + 1) * 0.01 + min_utility:
                        self.bidsBetweenUtility[i].append(b)
                        break
                limits += 1
    ...
    def bid_to_offer(self, state: MechanismState):
        ...
        if self.utility_function(best_opp_bid) >= self.utility_threshold or self.utility_function(best_opp_bid) >= minimum_of_bid:
            return best_opp_bid
    ...
    def accept_opponent_offer(self, opponent_bid, own_bid, time):
        current_utility = self.utility_function(opponent_bid)
        next_round_utility = self.utility_function(own_bid)
        ...
        if current_utility >= self.utility_threshold or current_utility >= next_round_utility:
            ...
            current_maximum_utility = self.utility_function(self.opponentBidHistory.get_best_bid_in_history()) * math.pow(self.discountingFactor, time)
            if current_maximum_utility > predict_maximum_utility and time > self.concedeToDiscountingFactor:
                if self.utility_function(opponent_bid) >= self.utility_function(self.opponentBidHistory.get_best_bid_in_history()) - 0.01:
                    return True
                else:
    ...
    def respond(self, state: MechanismState, offer: "Outcome") -> "ResponseType":
        ...
        self.opponentBidHistory.update_opponent_model(offer, self.utility_function(offer))
        ...
        if len(self.ownBidHistory.BidHistory) == 0:
            ...
            if self.estimate_round_left(True, state) > 10:
                ...
                if is_accept and not is_terminate:
                    ...
                    if self.utility_function(offer) > self.reservationValue:
                        action = ResponseType.ACCEPT_OFFER
                    else:
                        action = ResponseType.END_NEGOTIATION
                else:
                ...
                if state.relative_time > 0.9985 and self.estimate_round_left(True, state) < 5:
                    bid = self.opponentBidHistory.get_best_bid_in_history()
                    if self.utility_function(bid) < 0.85:
                    ...
                    if is_accept and not is_terminate:
                        ...
                        action = bid
        self.ownBidHistory.add_bid(bid, self.utility_function(bid))
...
class Atlas3(SAONegotiator):
    class NegotiationInfo:
        ...
        def set_value_relative_utility(self, max_bid):
            for issue in self.issues:
                current_bid = max_bid.copy()
                for value in issue.values:
                    current_bid[issue.name] = value
                    current_bid_util = self.utility_function(current_bid)
                    self.value_relative_utility[issue][value] = current_bid_util - self.utility_function(max_bid)

        def get_value_by_frequency_list(self, issue):
            max_f = 0
            max_value = None
            random_order_value = np.random.choice(issue.values, len(issue.values))
        ...
        def update_negotiation_info(self, offered_bid):
            ...
            util = self.utility_function(offered_bid)
        ...
        def update_pblist(self, popular_bid):
            if popular_bid not in self.pblist:
                ...
                self.mpbu = max(self.mpbu, self.utility_function(popular_bid))
                self.pblist.sort(key=self.utility_function)
    ...
    def on_ufun_changed(self):
        ...
        self.negotiation_info = Atlas3.NegotiationInfo(self.utility_function, self.ami)
    ...
    def choose_final_action(self, state: MechanismState, clist) -> "ResponseType":
        ...
        if self.offered_bid is not None:
            offered_bid_util = self.utility_function(self.offered_bid)
        ...
        cbid_util = self.utility_function(cbid)
    ...
    def get_bid(self, base_bid, threshold):
        ...
        if self.utility_function(bid) < threshold:
            bid = self.get_bid_by_appropriate_search(base_bid, threshold)
        if self.utility_function(bid) < threshold:
            bid = self.max_bid.copy()
    ...
    def relative_utility_search(self, threshold):
        ...
        random_issues = np.random.choice(self.negotiation_info.issues, len(self.negotiation_info.issues))
    ...
    def get_convert_bid_by_frequency_list(self, base_bid):
        ...
        for issue in random_issues:
            ...
            next_bid[issue.name] = self.negotiation_info.get_value_by_frequency_list(issue)
            if self.utility_function(next_bid) >= self.utility_function(current_bid):
    ...
    def select_accept(self, offered_bid, time):
        offered_bid_util = self.utility_function(offered_bid)
...
class AgentGG(SAONegotiator):
        ...
        def self_update(self, ordered_outcomes):
            ...
            for _, bid in ordered_outcomes[::-1]:
                ...
                for issue in bid:
                    ...
                    current_issue_list = self[issue]
                    for current_unit in current_issue_list:
                        if current_unit.value_of_issue == bid[issue]:
                            current_unit.weight_sum += current_weight
                            current_unit.count += 1
                            break
    ...
    def get_needed_random_bid(self, lower_ratio, upper_ratio):
        ...
        for t in range(3):
            ...
            returned_bid = None
            for bid in np.random.choice(self.ami.outcomes, self.n_outcomes):
    ...
    def propose(self, state: MechanismState) -> Optional[Outcome]:
        ...
        return self.get_needed_random_bid(self.offer_lower_ratio, self.offer_higher_ratio)
